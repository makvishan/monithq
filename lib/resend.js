...
const templates = {
  incidentCreated: (incident, site) => ({
    subject: `🚨 Incident Alert: ${site.name} is ${incident.status.toLowerCase()}`,
    html: `
      <!DOCTYPE html>
      <html>
        <head>
        ...
        <body>
          <div class="container">
            ...
            <div class="content">
              ...
              <p><strong>Severity:</strong> <span class="badge badge-${incident.severity.toLowerCase()}">${incident.severity}</span></p>
              <p><strong>Started:</strong> ${new Date(incident.startTime).toLocaleString()}</p>
              ${incident.aiSummary ? `<p><strong>AI Summary:</strong> ${incident.aiSummary}</p>` : ''}
              <a href="${process.env.NEXT_PUBLIC_APP_URL}/incidents" class="button">View Incident Dashboard</a>
            </div>
          </div>
        </body>
      </html>
        ...
        <body>
          <div class="container">
            ...
            <div class="content">
              ...
              <p><strong>Duration:</strong> ${Math.round(incident.duration / 60000)} minutes</p>
              <p><strong>Resolved:</strong> ${new Date(incident.endTime).toLocaleString()}</p>
              ${incident.aiSummary ? `<p><strong>Summary:</strong> ${incident.aiSummary}</p>` : ''}
              <a href="${process.env.NEXT_PUBLIC_APP_URL}/incidents" class="button">View Incident History</a>
            </div>
          </div>
        </body>
      </html>
  ...
  teamInvite: (email, inviterName, organizationName, role) => ({
        ...
        <body>
          <div class="container">
            ...
            <div class="content">
              ...
              <a href="${process.env.NEXT_PUBLIC_APP_URL}/auth/register?email=${encodeURIComponent(email)}" class="button">Accept Invitation</a>
            </div>
          </div>
        </body>
      </html>
        ...
        <body>
          <div class="container">
            ...
            <div class="content">
              ...
              <p style="margin-top: 30px; color: #666; font-size: 14px;">
                <a href="${process.env.NEXT_PUBLIC_APP_URL}/billing">View billing history</a>
              </p>
            </div>
          </div>
        </body>
      ...
      <html>
        ...
        <body>
          <div class="container">
            ...
            <div class="content">
              <p>Hi ${data.userName},</p>
              <p>We were unable to process your payment for your MonitHQ ${data.planName} subscription.</p>
      ...
      <html>
        ...
        <body>
          <div class="container">
            ...
            <div class="content">
              <p>Hi ${data.userName},</p>
              <p>Great news! Your plan has been upgraded from <strong>${data.oldPlan}</strong> to <strong>${data.newPlan}</strong>.</p>
              ...
              <div class="upgrade-box">
                ...
                <div class="feature">Team members: ${data.maxTeamMembers}</div>
                <div class="feature">Notification channels: ${data.channels}</div>
                ${data.aiCredits !== 'Unlimited' ? `<div class="feature">AI Credits: ${data.aiCredits}</div>` : '<div class="feature">Unlimited AI-powered insights</div>'}
              </div>

              <p>Your new features are active immediately. Start using them now!</p>
              
              <a href="${process.env.NEXT_PUBLIC_APP_URL}/dashboard" class="button">Explore New Features</a>
              
              <p style="margin-top: 30px; color: #666; font-size: 14px;">
                <a href="${process.env.NEXT_PUBLIC_APP_URL}/billing">View billing details</a>
              </p>
            </div>
          </div>
        </body>
...
export const sendEmail = async (to, template, data) => {
  ...
  try {
    ...
    } else if (['subscriptionCreated', 'paymentSucceeded', 'paymentFailed', 'subscriptionCanceled', 'subscriptionRenewal', 'subscriptionExpiring', 'planUpgraded'].includes(template)) {
  ...
  from: 'MonitHQ <onboarding@resend.dev>', // Change to your verified domain
      to: 'makvishan@gmail.com',
...
export const notifyTeamOfIncident = async (incident, site) => {
  try {
    ...
    const isOffline = incident.severity === 'HIGH' || incident.severity === 'CRITICAL';
    ...
    for (const member of teamMembers) {
      ...
      if (!member.notifyOnIncident) {
        console.log(`Skipping ${member.email}: incident notifications disabled`);
        continue;
      }

      // Check degradation preference
      if (isDegradation && !member.notifyOnDegradation) {
        console.log(`Skipping ${member.email}: degradation notifications disabled`);
        continue;
      }

      // Check admin-only preference
      if (member.notifyOnlyAdmins && member.role === 'USER') {
        console.log(`Skipping ${member.email}: only admins should receive notifications`);
        continue;
      }
      ...
      if (hasSubscriptions) {
        const isSubscribed = member.siteSubscriptions.some(sub => sub.siteId === site.id);
        if (!isSubscribed) {
      ...
      if (member.notifyViaSlack && allowedChannels.includes('slack') && member.slackWebhookUrl) {
        // TODO: Implement Slack notification
        console.log(`Would send Slack notification to ${member.email} (URL: ${member.slackWebhookUrl})`);
        sentChannels.slack++;
        memberSentCount++;
      }

      // Send via SMS if enabled and allowed
      if (member.notifyViaSms && allowedChannels.includes('sms') && member.smsPhoneNumber) {
      ...
      if (member.notifyViaWebhook && allowedChannels.includes('webhook') && member.customWebhookUrl) {
    ...
    console.log(`Sent ${sentCount} incident notifications: ${JSON.stringify(sentChannels)}`);
    return { success: true, sent: sentCount, total: teamMembers.length, channels: sentChannels };
  } catch (error) {
  ...
  try {
    ...
    for (const member of teamMembers) {
      ...
      if (!member.notifyOnResolution) {
        console.log(`Skipping ${member.email}: resolution notifications disabled`);
        continue;
      }
      ...
      // Send via Email if enabled and allowed
      if (member.notifyViaEmail && allowedChannels.includes('email')) {
        await sendEmail(member.email, 'incidentResolved', { ...incident, site });
        sentChannels.email++;
        memberSentCount++;
      }
      ...
      if (member.notifyViaSlack && allowedChannels.includes('slack') && member.slackWebhookUrl) {
        // TODO: Implement Slack notification
        console.log(`Would send Slack resolution to ${member.email} (URL: ${member.slackWebhookUrl})`);
        sentChannels.slack++;
        memberSentCount++;
      }
      ...
      if (member.notifyViaWebhook && allowedChannels.includes('webhook') && member.customWebhookUrl) {
        // TODO: Implement Webhook notification
        console.log(`Would send Webhook resolution to ${member.customWebhookUrl}`);
        sentChannels.webhook++;
        memberSentCount++;
      }
    ...
    console.log(`Sent ${sentCount} resolution notifications: ${JSON.stringify(sentChannels)}`);
    return { success: true, sent: sentCount, total: teamMembers.length, channels: sentChannels };
  } catch (error) {
...
const formatPlanFeatures = (planLimits) => {
  const features = {
    ...
    checkInterval: planLimits.minCheckInterval < 60 ? `${planLimits.minCheckInterval} seconds` : `${Math.floor(planLimits.minCheckInterval / 60)} minute${Math.floor(planLimits.minCheckInterval / 60) > 1 ? 's' : ''}`,
...
export default { sendEmail, notifyTeamOfIncident, notifyTeamOfResolution, formatAmount, formatPlanFeatures };
