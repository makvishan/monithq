...
const templates = {
  contact: (data) => ({
    ...
    html: `
      <div style="font-family:Arial,sans-serif;max-width:500px;margin:auto;padding:24px;border-radius:8px;background:#f9f9f9;">
        <h2 style="color:#6366f1;">MonitHQ Contact Request</h2>
        ...
        <p style="margin-top:24px;color:#888;font-size:14px;">This is an automated notification from MonitHQ.</p>
      </div>
    `,
  }),
  passwordReset: (data) => ({
  ...
  sslExpiry: (data) => ({
    subject: `${data.urgencyLevel === 'EXPIRED' ? '🔴' : data.urgencyLevel === 'CRITICAL' ? '⚠️' : '📋'} SSL Certificate ${data.urgencyLevel === 'EXPIRED' ? 'EXPIRED' : 'Alert'}: ${data.siteName}`,
    html: `
      <!DOCTYPE html>
      <html>
        <head>
          <style>
            body { font-family: Arial, sans-serif; line-height: 1.6; color: #333; }
            .container { max-width: 600px; margin: 0 auto; padding: 20px; }
            .header { background: ${data.urgencyLevel === 'EXPIRED' ? 'linear-gradient(135deg, #dc2626 0%, #991b1b 100%)' : data.urgencyLevel === 'CRITICAL' ? 'linear-gradient(135deg, #f59e0b 0%, #d97706 100%)' : 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)'}; color: white; padding: 30px; border-radius: 10px 10px 0 0; }
            .content { background: #f9f9f9; padding: 30px; border-radius: 0 0 10px 10px; }
            .badge { display: inline-block; padding: 8px 16px; border-radius: 20px; font-weight: bold; }
            ...
            .info-box { background: white; padding: 20px; border-radius: 8px; margin: 20px 0; border-left: 4px solid ${data.urgencyLevel === 'EXPIRED' ? '#dc2626' : data.urgencyLevel === 'CRITICAL' ? '#f59e0b' : '#667eea'}; }
            .button { display: inline-block; padding: 12px 24px; background: #667eea; color: white; text-decoration: none; border-radius: 6px; margin-top: 20px; }
          </style>
        </head>
        <body>
          <div class="container">
            <div class="header">
              <h1>${data.urgencyLevel === 'EXPIRED' ? '🔴 SSL Certificate EXPIRED' : data.urgencyLevel === 'CRITICAL' ? '⚠️ SSL Certificate Expiring Soon' : '📋 SSL Certificate Alert'}</h1>
            </div>
            <div class="content">
              <h2>${data.siteName}</h2>
              <p><strong>URL:</strong> ${data.siteUrl}</p>

              <div class="info-box">
                <p><strong>Status:</strong> <span class="badge badge-${data.urgencyLevel.toLowerCase()}">${data.daysRemaining < 0 ? 'EXPIRED' : `${data.daysRemaining} days remaining`}</span></p>
                <p><strong>Expiry Date:</strong> ${new Date(data.expiryDate).toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}</p>
                <p><strong>Issuer:</strong> ${data.issuer}</p>
              </div>

              ${data.daysRemaining < 0
                ? '<p><strong>⚠️ Action Required:</strong> Your SSL certificate has expired. Your site may show security warnings to visitors. Please renew your SSL certificate immediately.</p>'
                : data.daysRemaining < 7
                ? '<p><strong>⚠️ Urgent:</strong> Your SSL certificate is expiring in less than a week. Please renew it soon to avoid service interruption.</p>'
                : '<p><strong>Reminder:</strong> Your SSL certificate will expire soon. We recommend renewing it to ensure uninterrupted service.</p>'
              }

              <a href="${process.env.NEXT_PUBLIC_APP_URL}/sites" class="button">View Site Dashboard</a>
            </div>
          </div>
        </body>
      </html>
