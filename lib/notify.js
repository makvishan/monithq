...
import { INCIDENT_SEVERITY, NOTIFICATION_CHANNEL_IDS, USER_ROLES } from './constants.js';
...
export const notifyTeamOfIncident = async (incident, site) => {
  try {
    ...
    const isDegradation = incident.severity === INCIDENT_SEVERITY.MEDIUM;
    const isOffline =
    ...
    for (const member of teamMembers) {
      ...
      if (member.notifyOnlyAdmins && member.role === USER_ROLES.USER) {
        console.log(
      ...
      if (member.notifyViaEmail && allowedChannels.includes(NOTIFICATION_CHANNEL_IDS.EMAIL)) {
        await sendEmail(member.email, "incidentCreated", { ...incident, site });
        sentChannels[NOTIFICATION_CHANNEL_IDS.EMAIL]++;
        memberSentCount++;
      }
      ...
      if (
        ...
        allowedChannels.includes(NOTIFICATION_CHANNEL_IDS.SLACK) &&
...
// Send SSL certificate expiry notification to team members
export const sendSSLExpiryNotification = async (site, sslInfo) => {
  try {
    // Get organization subscription to check plan limits
    const organization = await prisma.organization.findUnique({
      where: { id: site.organizationId },
    ...
    const teamMembers = await prisma.user.findMany({
      ...
      select: {
        email: true,
        role: true,
        notifyOnIncident: true, // SSL expiry is treated as an incident
        notifyOnlyAdmins: true,
        // Channel preferences
        notifyViaEmail: true,
        notifyViaSlack: true,
    ...
    const urgencyLevel = daysRemaining < 0 ? 'EXPIRED' : daysRemaining < 7 ? 'CRITICAL' : daysRemaining < 14 ? 'WARNING' : 'NOTICE';

    // Filter and send notifications based on preferences
    for (const member of teamMembers) {
      // Check if user wants incident notifications (SSL expiry is treated as incident)
      if (!member.notifyOnIncident) {
        console.log(
          `Skipping ${member.email}: SSL notifications disabled`
        );
        continue;
      }
      ...
      // Send via Email if enabled and allowed
      if (member.notifyViaEmail && allowedChannels.includes("email")) {
        await sendEmail(member.email, "sslExpiry", {
          siteName: site.name,
          siteUrl: site.url,
      ...
      if (
        ...
        // Compose Slack message for SSL expiry
        const emoji = urgencyLevel === 'EXPIRED' ? '🔴' : urgencyLevel === 'CRITICAL' ? '⚠️' : urgencyLevel === 'WARNING' ? '⚡' : '📋';
        const statusText = daysRemaining < 0 ? 'EXPIRED' : `${daysRemaining} days remaining`;
        const slackMsg = `${emoji} SSL Certificate Alert for ${site.name}\nStatus: ${statusText}\nExpiry Date: ${new Date(sslInfo.validTo).toLocaleDateString()}\nIssuer: ${sslInfo.issuer}\nURL: ${site.url}`;
        await sendSlackNotification(member.slackWebhookUrl, slackMsg);
        sentChannels.slack++;
        memberSentCount++;
      }
      ...
      if (
        ...
        // TODO: Implement SMS notification
        console.log(`Would send SSL SMS to ${member.smsPhoneNumber}`);
        sentChannels.sms++;
        memberSentCount++;
      }
    ...
    console.log(
      `Sent ${sentCount} SSL expiry notifications: ${JSON.stringify(
        sentChannels
      )}`
    );
    return {
    ...
    console.error("Error sending SSL expiry notifications:", error);
    return { success: false, message: error.message };
  }
};
