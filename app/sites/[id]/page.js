...
export default function SiteDetailPage() {
  ...
  const fetchUptimeTrend = useCallback(async () => {
    try {
      ...
      const response = await fetch(
        `/api/sites/${params.id}/uptime-trend?period=${timeRange}`
      );
      const data = await response.json();
      if (response.ok) {
        setUptimeTrend(data);
  ...
  const fetchDistributions = useCallback(async () => {
    try {
      ...
      const response = await fetch(
        `/api/sites/${params.id}/distributions?limit=50`
      );
      const data = await response.json();
      if (response.ok) {
  ...
  const fetchTimeline = useCallback(async () => {
    try {
      ...
      const response = await fetch(
        `/api/sites/${params.id}/timeline?period=${period}`
      );
      const data = await response.json();
      if (response.ok) {
        setTimeline(data);
  ...
  const handleCheckNow = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const statusEmoji =
          data.check.status === "ONLINE"
            ? "✅"
            : data.check.status === "DEGRADED"
            ? "⚠️"
            : "❌";
      ...
      <MainContent>
              ...
              >
                <Card className="h-full  glow-ai">
                  <CardContent className="p-5">
                    ...
                    <p className="text-xs text-muted-foreground">
                      Active incidents: {summary?.activeIncidents || 0}
                    </p>
                  </CardContent>
                </Card>
              </motion.div>
                  ...
                  <CardContent>
                    <div className="p-4 rounded-lg bg-gradient-to-br from-purple-50 to-blue-50 dark:from-purple-900/20 dark:to-blue-900/20">
                      <p className="text-sm text-foreground whitespace-pre-wrap">
                        {summary.latestAIInsight.content ||
                          summary.latestAIInsight}
                      </p>
        ...
        {activeTab === "infrastructure" && (
          <>
            ...
            >
              <p className="text-xs text-muted-foreground">
                Refer here for more details on setting up and securing your  <a href="/assets/docs/MonitHQ-Infrastructure-Setup.md" download className="text-primary font-semibold hover:underline">Infra Setup</a>.
              </p>
            </motion.div>
                  ...
                  {loading.trend ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                      <div>
                        ...
                        <div className="flex items-baseline gap-2">
                          <p className="text-2xl font-bold text-foreground">
                            {uptimeTrend?.stats?.periodUptime?.toFixed(2) ||
                              "0.00"}
                            %
                          </p>
                          <span className="text-xs text-muted-foreground">
                        ...
                        <p className="text-xs text-muted-foreground mt-1">
                          Avg latency:{" "}
                          {uptimeTrend?.stats?.avgResponseTime || 0}ms
                        </p>
                        <div className="mt-3 h-2 bg-muted rounded-full overflow-hidden">
                          <div
                            ...
                            style={{
                              width: `${
                                uptimeTrend?.stats?.periodUptime || 0
                              }%`,
                            }}
                          ></div>
                        </div>
                      ...
                      <div>
                        ...
                        <div className="flex items-baseline gap-2">
                          <p className="text-2xl font-bold text-foreground">
                            {uptimeTrend?.stats?.totalChecks || 0}
                          </p>
                          <span className="text-xs text-muted-foreground">
                            checks
                          </span>
              ...
              <Card className=" glow-info">
                ...
                <CardContent>
                  {loading.trend ? (
                    ...
                    <div className="h-80">
                      {uptimeTrend &&
                      uptimeTrend.trendData &&
                      uptimeTrend.trendData.length > 0 ? (
                      ...
                      <div className="h-64">
                        ...
                        distributions.statusDistribution.length > 0 ? (
                          <ResponsiveContainer width="100%" height="100%">
                            <PieChart>
                              <Pie
                                data={distributions.statusDistribution.map(
                                  (item) => ({
                                    name: item.name,
                                    value: item.value,
                                    fill:
                ...
                <Card className="h-full glow-warning">
                  ...
                  <CardContent>
                    {loading.distributions ? (
                      ...
                      <div className="h-64">
                        {distributions &&
                        distributions.responseTimeDistribution &&
                        distributions.responseTimeDistribution.length > 0 ? (
                          <ResponsiveContainer width="100%" height="100%">
                            <BarChart
                            ...
                            >
                              ...
                              <Bar dataKey="value" radius={[8, 8, 0, 0]}>
                                {distributions.responseTimeDistribution.map(
                                  (entry, index) => {
                                    const colors = {
                                      "Fast (< 100ms)": "#22c55e",
                        ...
                        .map((check, i) => {
                          ...
                          return (
                            <div
                              ...
                              title={`${check.status} - ${
                                check.responseTime
                              }ms at ${formatTimestamp(check.checkedAt)}`}
                            ></div>
                          );
                        })}
                    </div>
                ...
                <CardContent>
                  ...
                  {checks.length === 0 && (
                    <div className="text-center py-12 text-muted-foreground">
                      No checks recorded yet. Click &quot;Check Now&quot; to
