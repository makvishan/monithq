              ...
              <div className="w-10 h-10 rounded-full gradient-ai flex items-center justify-center text-white font-semibold glow-primary shrink-0 relative">
                {currentUser?.name
                  ? currentUser.name
                      ...
                      .toUpperCase()
                  : "JD"}
                {/* Show shield only if email is verified */}
                {currentUser && currentUser.emailVerified && (
                  <div
                    className={cn(
                      "absolute -top-1 -right-1 w-4 h-4 rounded-full border-2 border-card flex items-center justify-center bg-green-500"
                    )}
                  >
